...
public interface SupplierDao extends JpaRepository<Supplier, String> {
	...
	@Query(value = "update supplier"
			...
			+ " `name` = (case when :input_name is null then `name` else :input_name end),"
			+ " compiled = (case when :input_compiled is null then compiled else :input_compiled end),"
			+ " email = (case when :input_email is null then email else :input_email end),"
			+ " phone = (case when :input_phone is null then phone else :input_phone end),"
			+ " location_id = :input_location_id,"
			+ " location_name = (case when :input_location_name is null then location_name else :input_location_name end)"
			+ " where serial_num = (case when :input_serial_num is null then serial_num else :input_serial_num end)",nativeQuery = true)
